...
static int RunVVIS( const std::filesystem::path& p2ceGameDir, const std::filesystem::path& bspPath, const std::filesystem::path& p2ceBinDir )
{
    ...
    if (exitCode != 0)
    {
        ...
        //! While VVIS errors, PeTI is programmed to ignore that error and continue. This behavior is not caused by BEEMod.
        //! We must return 0 for VVIS whenever it error to not stop due to leaks so BEEMod's VRAD error server can take over.
        return 0;
    }
...
static int RunVRAD( const std::filesystem::path& p2ceGameDir, const std::filesystem::path& bspPath, const std::filesystem::path& p2ceBinDir, const bool usingBEE )
{
    ...
    if (usingBEE)
    {
        ...
        {
            Log("[RunVRAD] Failed to start BEEMod VRAD!", true);
    ...
    Process vradP2CE;
    if (!vradP2CE.Start(
    std::format(R"({}/vrad.exe -final -staticproppolys -staticproplighting -textureshadows -hdr -portaltraversallighting -portaltraversalao -game "{}" "{}")", p2ceBinDir.string(), p2ceGameDir.string(), bspPath.string()),
    "",
    [](const std::string& text, const bool isSTDErr)
    {
        // Can't use Log functions as it will add another return carriage.
...
int main( const int argc, char* argv[] )
{
    ...
    if (execName == "vbsp")
    {
        ...
        if (!std::filesystem::exists(p2ceGameBaseDir / "bin/bee2"))
